...
BOT_TOKEN = '5717600255:AAHC1mn_MTCROaN_fOjI-7WAPH5HJWzbMvA'
BOT_NAME = 'calculator_bot'
bot = telebot.TeleBot(BOT_TOKEN)

TIMEOUT_CONNECTION = 5

START_MESSAGE = """Что будем вычислять? (:"""
HELP_MESSAGE = """Всё очень просто. Вы отправляете мне необходимое вычисление и получаете его результат.
...
def answer_to_user(message):
    ...
    if user_message == 'привет':
        ...
        try:
            ...
            msg = bot.send_message(message.chat.id, 'Что-то пошло не так. Может, неправильно введено выражение?. \nПопробуйте ещё раз.')
        except NameError:
            msg = bot.send_message(message.chat.id, 'Таких переменных нет. \nПопробуйте ещё раз.')
        except TypeError:
            msg = bot.send_message(message.chat.id, 'Похоже, что в выражении присутствует ошибка типов. \nПопробуйте ещё раз.')
        except ZeroDivisionError:
            msg = bot.send_message(message.chat.id, 'Делить на ноль нельзя! Просто напоминание (:. \nПопробуйте ещё раз.')
